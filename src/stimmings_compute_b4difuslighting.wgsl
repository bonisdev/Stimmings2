...
var lightPersuX: f32 = 0f;  //accum<-X
var lightPersuY: f32 = 0f;  //accum<-Y

var lightNebgsX: array<f32, 8> = array<f32, 8>(0,0,0,0,  0,0,0,0);
var lightNebgsY: array<f32, 8> = array<f32, 8>(0,0,0,0,  0,0,0,0);

var onTargetBooster: u32 = 8u;
...
loop {                              // Goes 0-7 (inclusive)
    ...
    if( fhelper > 0u ){
        ...
        fhelper *= onTargetBooster;         // NOTE*** applying this only when changing the direction vector
        //finalLightsVerdict = 1u;
        lightPersuX += (f32(fhelper)/255f) * (-1f + f32((8u-di)%3u));
        lightPersuY += (f32(fhelper)/255f) * (-1f + f32((8u-di)/3u));

    }


    // does the INTENsitt of the neibhgour hav eosmethign
    else if( ((tmpcrvl >> 24u) & 0x000000FFu) > 0u ) {




        // Get the neighbours light beam
        var dirX: f32 = -1f + (f32(tmpcrvl & 0x00000FFFu) / 4095f)*2f;
        var dirY: f32 = -1f + (f32((tmpcrvl >> 12u) & 0x00000FFFu) / 4095f)*2f;
        var intensityfg: u32 = (tmpcrvl >> 24u) & 0x000000FFu;
        // NOW these vals are the neighbours LIGHT source (-1 to 1)
        if( intensityfg > highestIntensit ){
            highestIntensit = intensityfg;
        }



        totallightintensity += intensityfg;


        if(intensityfg > 0u){

            // HAS LANDED ON ME!!!
            if( (8-di) == convert_to_di(dirX, dirY) ){  //<- if hits ME AND has intensity 
                directLightHits += 1f;
                intensityfg *= onTargetBooster;     // NOTE*** applying this only when changing the direction vector 
            }
            else{
                lightMisses += 1f;
            }
            ...
            lightPersuX += (f32(intensityfg)/255f) * dirX;
            lightPersuY += (f32(intensityfg)/255f) * dirY;
...
if( totallightintensity > 0u && entityType < 1u && directLightHits > 0){ // ANY movement on me..

    // Figur out how much subtraction


    // SUBRTTACTONE  from highest
    if( highestIntensit > 0u ){
        // no *DIRECT rays on me
        if( directLightHits < 1f ){
            lightMisses += 9f;       // this essentially causes squares that didnt exactly get
                // a sitting on match of their neighbours direction beam take a larger hit 
                // (we still give them the aggregated light direction from the neighbours
                //      just lower their intensity extrea much so their direction matters less anyways )
        }
        var othervar: f32 = 1f;
        //BUT could still be a lot around me
        othervar = (f32(totallightintensity) / (directLightHits+lightMisses)) / 1;
        
        // Strongest intensity is weighted 7x as much as the AVERAGE intensity
        highestIntensit = u32( max(0, ( f32(highestIntensit*1) ) * (0.7935f - lightMisses*0.007f) ) );// - 1u;
        //highestIntensit = highestIntensit - 1u;
    }
    else{
    ...
    var pckedX: u32 = u32((noWeightDir.x+1f)*0.5f*4095f);
    var pckedY: u32 = u32((noWeightDir.y+1f)*0.5f*4095f);
